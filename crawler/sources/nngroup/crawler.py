...
from crawler.sources.nngroup.parser import (
...
class NNGroupCrawler(BaseCrawler):
    """Crawls UX research articles from Nielsen Norman Group."""
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append(
                    "No article links found in NN/g RSS feed."
                )
                return result
