...
from crawler.sources.designmilk import config
from crawler.sources.designmilk.parser import (
...
class DesignMilkCrawler(BaseCrawler):
    """Crawls product and interior design articles from Design Milk."""

    source_name = "designmilk"
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append(
                    "No article links found in Design Milk RSS feed."
                )
                return result
