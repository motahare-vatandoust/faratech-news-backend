...
from crawler.sources.bensbites.parser import (
...
class BensBitesCrawler(BaseCrawler):
    """Crawls articles from Ben's Bites (Substack)."""

    source_name = "bensbites"
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append("No article links found in Ben's Bites RSS feed.")
                return result
