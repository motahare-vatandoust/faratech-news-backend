...
from crawler.sources.huggingface import config
from crawler.sources.huggingface.parser import (
...
class HuggingFaceCrawler(BaseCrawler):
    """Crawls articles from the Hugging Face blog."""
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append(
                    "No article links found in Hugging Face RSS feed."
                )
                return result
