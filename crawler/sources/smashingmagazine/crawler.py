...
from crawler.sources.smashingmagazine import config
from crawler.sources.smashingmagazine.parser import (
...
class SmashingMagazineCrawler(BaseCrawler):
    """Crawls design and UX articles from Smashing Magazine."""

    source_name = "smashingmagazine"
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append(
                    "No article links found in Smashing Magazine RSS feed."
                )
                return result
