...
from crawler.sources.marketingweek.parser import (
    extract_article_urls,
    parse_article_page,
)


class MarketingWeekCrawler(BaseCrawler):
    """Crawls articles from marketingweek.com."""

    source_name = "marketingweek"
    ...
    async def crawl(self, *, limit: Optional[int] = None) -> CrawlResult:
        ...
        async with crawl_client() as fetcher:
            ...
            if not article_urls:
                result.errors.append(
                    "No article links found on Marketing Week homepage."
                )
                return result
